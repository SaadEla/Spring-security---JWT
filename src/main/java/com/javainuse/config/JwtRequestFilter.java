...
public class JwtRequestFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		System.out.println("ha lewla"+requestTokenHeader);
		...
		if (requestTokenHeader != null && requestTokenHeader.startsWith("Bearer ")) {
			...
			System.out.println("Ha tanya"+jwtToken);
			try {
				username = jwtTokenUtil.getUsernameFromToken(jwtToken);
				System.out.println("L9it token o khdit username"+username);
		...
		if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
			System.out.println("----Etape validation Token------");
			UserDetails userDetails = this.jwtUserDetailsService.loadUserByUsername(username);
			System.out.println("Khdina user detail"+userDetails);
			...
			if (jwtTokenUtil.validateToken(jwtToken, userDetails)) {
				...
				System.out.println("------Hana1"+usernamePasswordAuthenticationToken);
